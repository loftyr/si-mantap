...
class m_tbl_pensiun extends CI_Model
{
    ...
    private function _get_datatables_query($Where)
    {
        ...
        if ($Where["Status"] != "") {
            $this->db->where('DATEDIFF(DATE_ADD(Tanggal_Lahir, INTERVAL ' . $Where["Pensiun"] . ' YEAR), CURDATE()) ' . $Where["Status"] . '');
        }
    ...
    {
        $this->db->select('*');
        $this->db->select('DATE_ADD(Tanggal_Lahir, INTERVAL ' . $Where["Pensiun"] . ' YEAR) AS Pensiun');
        $this->db->select('DATEDIFF(DATE_ADD(Tanggal_Lahir, INTERVAL ' . $Where["Pensiun"] . ' YEAR), CURDATE()) AS Delay');
